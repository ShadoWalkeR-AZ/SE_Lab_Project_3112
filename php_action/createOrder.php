  ...
  $paymentStatus 	= $_POST['paymentStatus'];
	...
	for($x = 0; $x < count($_POST['productName']); $x++) {			
		...
		while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
		    $quantity_set =  $_POST['quantity'][$x]-$_POST['return'][$x];
			$updateQuantity[$x] = $updateProductQuantityResult[0] - $quantity_set;
				// update product table
				$updateProductTable = "UPDATE product SET quantity = '".$quantity_set."' WHERE product_id = ".$_POST['productName'][$x]."";
				$connect->query($updateProductTable);

				// add into order_item
				$orderItemSql = "INSERT INTO order_item (order_id, product_id, quantity, rate, total, order_item_status) 
				VALUES ('$order_id', '".$_POST['productName'][$x]."', '".$quantity_set."', '".$_POST['rateValue'][$x]."', '".$_POST['totalValue'][$x]."', 1)";
