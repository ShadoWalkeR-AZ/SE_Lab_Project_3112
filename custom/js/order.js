	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
            ...
            var ret = document.getElementsByName('return[]');
            var validateRet;
            for (var x = 0; x < ret.length; x++) {
                var retId = ret[x].id;
                if(ret[x].value == ''){
                    $("#"+retId+"").after('<p class="text-danger"> Product Name Field is required!! </p>');
                    $("#"+retId+"").closest('.form-group').addClass('has-error');
                } else {
                    $("#"+retId+"").closest('.form-group').addClass('has-success');
                }
            }  // for
			...
			if(orderDate && clientName && clientContact && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true && validateRet == true) {
					// create order button
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				'</td>'+
                 ...
                 '<td style="padding-left:20px;">'+
                 '<div class="form-group">'+
                 '<input type="number" name="return[]" id="return'+count+'" onkeyup="getTotal('+count+')" autocomplete="off" class="form-control" min="0" />'+
                 '</div>'+
                 '</td>'+

				 '<td style="padding-left:20px;"">'+
            	 '<input type="text" name="sale[]" id="sale'+count+'" autocomplete="off" disabled="true" class="form-control" />'+
            	 '<input type="hidden" name="saleValue[]" id="saleValue'+count+'" autocomplete="off" class="form-control" />'+
            	 '</td style="padding-left:20px;  ">'+
...
function getTotal(row = null) {
	if(row) {
        var sale = Number($("#quantity"+row).val())-Number($("#return"+row).val());
        sale = sale.toFixed(0);
        $("#sale"+row).val(sale);
        $("#saleValue"+row).val(sale);

		var total = Number($("#rate"+row).val()) * sale;
		total = total.toFixed(2);
		$("#total"+row).val(total);
		$("#totalValue"+row).val(total);
